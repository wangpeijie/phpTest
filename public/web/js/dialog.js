	...
	var options=$.extend(true,opt,opts),
		...
		buildDialog=function(){
			...
			if(options.title||options.closeText){
				dialogHtml+='<div class="dialog_head p18">';
							
			if(options.closeText){
					dialogHtml+='<a href="javascript:;" class="closeBtn fr" role="dialogCloseBtn" '+(!options.showCloseText?'style="display:none;"':'')+'>'+
								stringToIcon(options.closeText)+
								'</a>';
				};
				
				if(options.title){
					dialogHtml+='<div class="dialogTitle" role="dialogTitle">'+options.title+'</div>';
				};
									
				dialogHtml+='</div>';
			};
			
			dialogHtml+='<div class="dialog_body">'+
								'<div class="wrapper clearfix">'+
								'</div>';
							
			if(options.confirmText||options.cancelText){			
				dialogHtml+='<div class="dialog_foot tar"><div class="edit_btnBox clearfix">';
								
				if(options.confirmText){
					dialogHtml+='<a class="mainBtn iBc" href="javascript:;" role="dialogConfirmBtn" '+(!options.showConfirmText?'style="display:none;"':'')+'>'+stringToIcon(options.confirmText)+'</a>';
				};
				
				if(options.cancelText){
					dialogHtml+='<a class="addBtn ml10" href="javascript:;" role="dialogCancelBtn" '+(!options.showCancelText?'style="display:none;"':'')+'>'+stringToIcon(options.cancelText)+'</a>';
				};
...
//						dialog.find('div.boxscroll').remove();
...
//		html+='<li><a href="javascript:" data-value="'+item.value+'"><i class="'+(item.icon?item.icon:'icon-menudefault')+'"></i><span>'+item.name+'</span></a></li>';
//	});
//	html+='</ul>';
//	
//	
