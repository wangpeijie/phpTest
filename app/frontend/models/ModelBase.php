...
class ModelBase extends \Phalcon\Mvc\Model
{
        ...
        public function batchInsert($params)
        {
                ...
                foreach ($params as $value)
                {
                        ...
                        foreach ($values as $k => $value2)
                        {
                                $values[$k] = "'" . addcslashes(str_replace("'", "''", $value2), "\000\n\r\\\032") . "'";
                        }
                        $sql2[] = '(' . implode(',', $values) . ')';
                }
                $sql .= implode(',', $sql2);
        ...
        public function batchInsert2($params)
        {
                foreach ($params as $value)
                {
                        if ($value)
                ...
                foreach ($params as $k => $v)
                {
                        foreach ($keys as $v2)
                        {
                                if (isset($v[$v2]))
                                {
                                        $p[$k][$v2] = "'" . addcslashes(str_replace("'", "''", $v[$v2]), "\000\n\r\\\032") . "'";
                                } else
                                {
                                        $p[$k][$v2] = "";
                                }
        ...
        public function batchInsert3($columns, $params)
        {
                $db = getDb();
                ...
                foreach ($params as $k => $v)
                {
                        foreach ($v as $v2)
                        {
                                $params[$k][] = "'" . addcslashes(str_replace("'", "''", $v2), "\000\n\r\\\032") . "'";
                        }
                        $sql2[] = '(' . implode(',', $params[$k]) . ')';
                }
                $sql .= implode(',', $sql2);
                return $db->execute($sql);
        }
